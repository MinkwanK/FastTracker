...
def filter_vehicle_detections(outputs):
    """
    Filter detections to only include vehicle classes.
    Detection format: (x1, y1, x2, y2, obj_conf, class_conf, class_pred)
    Returns filtered detections with same format.
    """
    if outputs is None or len(outputs) == 0:
        return outputs
    
    # Check if we have class predictions (shape[1] should be 7 for full detections)
...
def image_demo(predictor, vis_folder, current_time, args, exp):
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            ...
            filtered_outputs = filter_vehicle_detections(outputs[0])
            
            if filtered_outputs is not None and len(filtered_outputs) > 0:
                online_targets = tracker.update(filtered_outputs, [img_info['height'], img_info['width']], exp.test_size)
                online_tlwhs = []
                online_ids = []
                online_scores = []
                ...
                online_im = plot_tracking(
                    img_info['raw_img'], online_tlwhs, online_ids, frame_id=frame_id, fps=1. / timer.average_time, roi_points=getattr(tracker, "roi_points", [])
                )
            else:
                timer.toc()
...
def imageflow_demo(predictor, vis_folder, current_time, args, exp):
    ...
    logger.info("=" * 60)
    logger.info("Vehicle Detection Mode - Only tracking vehicles")
    logger.info("Classes: bicycle, car, motorcycle, bus, train, truck")
    logger.info("=" * 60)
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                if filtered_outputs is not None and len(filtered_outputs) > 0:
                    ...
                    timer.toc()
                    online_im = plot_tracking(
                        img_info['raw_img'], online_tlwhs, online_ids, frame_id=frame_id + 1, fps=1. / timer.average_time, roi_points=getattr(tracker, "roi_points", [])
                    )
                else:
                    timer.toc()
                    online_im = img_info['raw_img']
                ...
                timer.toc()
                online_im = img_info['raw_img']
            cv2.imshow('Vehicle Tracking', online_im)
            if args.save_result:
